      ...
      <section className="bg-gradient-to-br from-slate-50 to-blue-50 py-16 border-b border-blue-100">
        <div className="container mx-auto px-6 text-center">
          ...
          <div className="flex gap-4 justify-center flex-wrap">
            <a href="https://wa.me/201062485133" className="bg-[#107060] text-white px-7 py-3 rounded-xl font-bold hover:bg-[#084941] transition-colors shadow">اشترك في الدعم الآن</a>
      ...
      <main className="container mx-auto px-6 py-14 max-w-5xl">
        ...
        <section className="mb-14">
          ...
          <div className="grid md:grid-cols-3 gap-5">
            {packages.map((pkg) => (
              <div key={pkg.name} className="p-5 bg-white border border-slate-200 rounded-xl hover:border-[#2DBEA1] transition-colors">
                <h3 className="font-bold text-slate-800 mb-1">{pkg.name}</h3>
                <p className="text-xs text-[#2DBEA1] mb-4">{pkg.suitable}</p>
                <ul className="space-y-2">
                  {pkg.features.map((f) => (
                    <li key={f} className="flex items-center gap-2 text-sm text-slate-600">
                      <span className="w-1.5 h-1.5 bg-[#2DBEA1] rounded-full flex-shrink-0" />
                      {f}
                    </li>
                  ))}
                </ul>
        ...
        <section className="mb-14">
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            <Link href="/services/website" className="p-4 bg-white border border-slate-200 rounded-xl hover:border-[#2DBEA1] hover:shadow transition-all text-slate-700 font-medium text-sm">🌐 تصميم موقع ويب جديد</Link>
            <Link href="/services/mobile-app" className="p-4 bg-white border border-slate-200 rounded-xl hover:border-[#2DBEA1] hover:shadow transition-all text-slate-700 font-medium text-sm">📱 تطوير تطبيق جوال جديد</Link>
            <Link href="/services/custom-system" className="p-4 bg-white border border-slate-200 rounded-xl hover:border-[#2DBEA1] hover:shadow transition-all text-slate-700 font-medium text-sm">🔧 نظام مخصص جديد</Link>
